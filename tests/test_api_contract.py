...
def test_metrics_overview_endpoint_returns_sections():
    response = client.get("/metrics/overview")
    assert response.status_code == 200
    payload = response.json()
    assert "model_comparison" in payload
    assert "operational" in payload
    assert "volume" in payload["operational"]
    assert "latency" in payload["operational"]
    assert "explainability" in payload["operational"]
    assert "rag" in payload["operational"]
    assert "llm" in payload["operational"]
